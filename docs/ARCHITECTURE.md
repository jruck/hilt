...
Server watchers detect filesystem changes:
  ...
  library-watcher.ts → watches saved refs, hidden candidates, processing queue,
                       recommendation state, and recommendation context signals
...
`Needs attention` is a derived cross-lifecycle review lens rather than another persisted lifecycle. On each Library read, `attention.ts` joins current artifact state to `${DATA_DIR}/library-refetch-attempts/<vault-hash>.json`: any `processing.state = blocked` item qualifies, as does a legacy capture-health failure after the standard two refetch attempts. Healed items and captures still inside their retry budget do not qualify. The API exposes the projection through `attention=true`; the workbench exposes an `attention` facet for the rail count. A source can be acknowledged as `unavailable` or `accepted_limited` in `${DATA_DIR}/library-source-resolutions/<vault-hash>.json`; this removes it from Needs attention and automatic refetch while leaving capture health failed, the item unscored, and Markdown untouched. For blocked processing, the exact terminal queue payload must move to `${DATA_DIR}/library-source-resolution-queue/<vault-hash>/` before the disposition ledger is written; mismatches fail with no decision, archive failures leave the queue live, and a later ledger failure restores the payload. A live blocked queue always wins over a stale disposition when deriving attention. When capture health recovers, its disposition is pruned so a later regression cannot reuse an old acknowledgement. Retry can restore an archived payload exactly. Legacy exhausted captures instead clear only their attempt cap and truthfully wait for the next bounded refetch. Refetch prunes attempt records that are healed, deleted, archived, or acknowledged.

Queue health classifies a sidecar as stale only when its vault, stable artifact UID, target path, and ready Markdown state all agree. Read paths exclude that residue immediately; `library:health:reconcile` is dry-run by default and requires `--write` to remove it. The backend emits one stable `issues[]` contract with `critical`, `warning`, and `working` severities plus authoritative counts. Critical means a capability is unavailable, warning means intervention is needed, and working means Hilt is already processing or waiting for its scheduled pass; the client does not recalculate severity.

Library scheduler stdout/stderr logs remain append-only diagnostics, but recommendation health is governed by sanitized attempt receipts in `runtime.json`; a later success supersedes older stderr automatically, and retained recommendation stderr is never returned through the health API. Claude print prompts are removed from process arguments and sent over stdin, thrown errors carry only a context-free status, and scheduler installation creates the log directory/files at owner-only `0700`/`0600` permissions.
...
```
ready study artifacts + 72h context evidence
  (meetings, tasks, projects, areas, briefings, recent saves)
                     │ 5:20 AM editor or meaningful on-demand refresh
                     ▼
          strict 0–12 episode selection
                     │ validate supplied trigger fingerprints, cooldowns,
                     │ duplicate content, and materially changed pitches
                     ▼
immutable batch → atomic feed projection → cursor API → Library For You
                           │                         └→ WebSocket live insertion
                           └→ briefing selector (up to 3 new unread episodes)
                                                     └→ `rec:<episode-id>` row
```
...
The scheduler runs the main batch at 5:20 AM, before the 6:00 briefing. `ws-server` owns a non-resident child runner for meaningful refreshes: one newly ready explicit save, three distinct newly ready candidates, or a strong novel context match can mark a refresh pending; runs debounce for 20 minutes and require two hours between automated batches. Cadence reserves at most one morning attempt plus one refresh attempt per local day (`America/New_York` by default) before model work begins, so a rate limit or invalid response cannot retry every hour; legacy success counters are interpreted conservatively during migration. Transcript files are excluded because they are not supplied to the editor, including persisted pre-cutover triggers. Validation also rejects a near-duplicate title from the recent seven-day exposure window while allowing the same artifact to resurface only with materially new evidence. Batch, dismissal, and restore writes emit `library:recommendations-changed`; the client revalidates cursor pages, deduplicates by stable artifact id, preserves a deep-scroll anchor, and reports impressions only when an episode actually becomes visible.

The artifact and recommendation layers deliberately own different copy. Markdown `description` is an evergreen, source-centric account of what an item contains. Episode `why_now` is a contextual recommendation pitch grounded in changed timing, evidence, a decision, or an actionable consequence. Editor validation rejects a pitch whose token similarity to the source description is `0.65` or higher and requires context-triggered pitches to carry meaningful language from a non-artifact trigger. Normal Library list/detail reads cheaply join the current non-dismissed episode as `LibraryArtifact.recommendation`; recommendation history remains Hilt-local and does not rewrite the source note.

Briefing markdown freezes placement with token-only `rec:<episode-id>` lines. The canvas passively hydrates those exact episodes without logging an open, renders compact recommendation cards, and navigates into Library with `rank=for-you&rec=<episode-id>` only when clicked. Briefing and For You cards use the stored episode pitch as their sole unlabeled description; Recent/New/Search use only the source description and expose active recommendation membership as a quiet sparkle signal. An ordinary detail open shows the current active pitch, while a `rec` link hydrates the exact historical episode before the normal source digest. Later batches do not rewrite that morning's selected episode IDs.

The generated `Library & knowledge` briefing section is partitioned into three ordered modules: `Recommended for you` (an editorial set lead, up to three frozen episode cards, and attached `View all`), optional Saturday-anchored weekend `Editor's memo`, and quiet daily `Library health`. The memo job runs Saturday at 5:30 AM before the first weekend briefing; Sunday may update that same Saturday edition, while weekdays omit the module. Health comes from the exact date's deterministic steering summary and never links to an older report when today's report is unavailable. Historical briefings without the module headings retain the generic markdown renderer.
...
Playlist/course ingestion uses the same child-item pipeline. A YouTube playlist source can declare `metadata.series_id`/`series_title`/`series_parent`; the adapter stamps each fetched video with playlist provenance plus 1-based `series_index`, digestion threads that into `ProcessedArtifact.series`, and both saved references and candidates persist the `series_*` frontmatter. The parent is a normal `type: reference` note (`format: video-series`, `series_role: parent`) generated by `scripts/library-series-synthesize.ts`: dry-run by default, `--write` to create/update the map, and optional `--analyze` to run the existing read-only reweave engine across the child summaries for whole-series synthesis. This keeps transcripts/digests/connections item-level while giving the playlist/course a durable synthesis object.
...
| Calendar cache | `${DATA_DIR}/calendar.sqlite` + `fantastical-evercommerce-calendar.json` | SQLite + private derived snapshot | Read-only normalized ICS events. An explicit foreground command normalizes Fantastical's protected cache into a participant-only snapshot; EverCommerce sync enriches from that Hilt-owned snapshot by UID/occurrence. ICS remains authoritative and enrichment provenance stays in `raw_json`. |
...
- In production: custom `server/app-server.ts` serves a production Next build
  from `.next-prod`; Next standalone output is distribution-only
...
dir via `HILT_DIST_DIR`, so the production server never reads `.next`. Prod is the
...
and those builds clean stale generated artifacts before tracing so old `.next`/`dist`
contents cannot be copied recursively into a new bundle. `npm run clean:build-artifacts`
is the explicit generated-output cleanup path; it removes Next build dirs and
electron-builder output but leaves source, `node_modules`, vault data, and Hilt user data
alone. Forced cleanup refuses to run while the Hilt supervisor heartbeat is fresh unless
`--allow-running` is passed.
