...
- BE THOROUGH. Before deciding, search his vault widely for the reference's core concepts and vocabulary — do NOT rely on the index alone, which cannot list every note. A connection you never looked for is a connection missed.
- Report EVERY genuine connection you find — there is NO upper limit and no quota. A substantive reference that touches a well-developed strand of his work may genuinely connect to 8-12 or more notes; surface all of them, each with its own specific target and directional sentence. The gate is genuineness, not quantity: never invent or pad a tie, but never omit a real one either. Reasons must be relationship claims, not topic labels ("extends X with a delivery-rate counterexample", not "related to AI").
