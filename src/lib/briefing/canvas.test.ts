...
import {
  ...
  isRecommendationEpisodeOnlyLine,
  isTaskIdOnlyLine,
...
test("recommendation episode tokens parse, strip, and recognize token-only placement lines", () => {
  const one = "rec-20260710052000-01-ab12cd34";
  const two = "rec-20260710052000-02-ef56ab78";
  assert.deepEqual(extractRecommendationEpisodeIds(`- \`rec:${one}\` and rec:${two} then rec:${one}`), [one, two]);
  assert.equal(isRecommendationEpisodeOnlyLine(`- \`rec:${one}\``), true);
  assert.equal(isRecommendationEpisodeOnlyLine(`- Read this \`rec:${one}\``), false);
  assert.equal(stripRecommendationTokens(`Why \`rec:${one}\``).trim(), "Why");
});

test("Library subheadings partition the three briefing modules without changing flat legacy sections", () => {
  const structured = parseBriefing(`# Briefing

## 📚 Library & knowledge

### Recommended for you

These ideas sharpen the same delivery decision without repeating the cards below.

- \`rec:rec-20260711052000-01-ab12cd34\`

### Editor's memo

**The weekly argument.** A concise summary.

[Read the memo](/api/reports/memo)

### Library health

Library intake is healthy.

[Daily library report](/api/reports/morning)
`).sections[0];
  const partition = partitionBriefingLibrarySection(structured);
  assert.equal(partition.structured, true);
  assert.equal(partition.modules.recommendations?.items.length, 2);
  assert.equal(partition.modules.memo?.items.length, 2);
  assert.equal(partition.modules.health?.items.length, 2);
  assert.equal(partition.ungrouped.length, 0);

  const legacy = partitionBriefingLibrarySection(parseBriefing(SAMPLE).sections[2]);
  assert.equal(legacy.structured, false);
  assert.equal(legacy.ungrouped.length, 2);
});
...
test("isDecisionsHeading distinguishes the live Decisions section from legacy Next steps", () => {
  assert.equal(isDecisionsHeading("⏭ Decisions awaiting you"), true);
  assert.equal(isDecisionsHeading("⏭️ Decisions awaiting you"), true);
  assert.equal(isDecisionsHeading("⏭ Next steps"), false);
});
