...
- **System Agent mode implementation plan** — Added `docs/plans/system-agent-mode.md`, a docs-only handoff plan for a future observer runtime that lets machines such as Hestia report System Sync, Apps, Sessions, and Stack status without running the full Hilt UI, product daemons, or write-capable app surfaces.
