...
- Header includes a compact health panel backed by `/api/library/health`. Its badge totals every actionable blocker and warning, and each counted issue must map to a visible status row: Runtime covers recommendations, dead letters, intake, and reweave; Scheduler and Sources retain per-row status and expandable evidence. Active queues are blue/informational rather than warnings. Health refresh is status-only; the separate `Check sources` action runs a bounded live ingest for the selected source or all hourly sources, then revalidates the local Library view.
