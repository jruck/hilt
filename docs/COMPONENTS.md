...
- Terminal source failures expose honest item-level actions in `LibraryArtifactDetailPane`: Retry immediately restores a resumable processing payload or reports that a legacy refetch was reset for the next scheduled pass; `Keep limited capture` and `Mark source unavailable` stop retries and remove the item from Needs attention without giving it a score. The acknowledged card keeps a `Try source again` path.
...
- Detail panes expose one comment icon, not separate comment/chat actions. Sending from its popover appends an asynchronous queued comment; `Chat now` expands that same anchored popover into the live conversation. The full Chats pane is an explicit expansion, not a prerequisite for model work.
...
- Header includes a compact health panel backed by `/api/library/health`. The backend owns severity and counts; the client renders its stable `Needs attention` and `Working or scheduled` issue rows instead of independently recounting scheduler/source/queue fields. Red means a capability is unavailable, amber requires intervention, and blue means Hilt is already handling the work. Runtime calls the retained Connections pass **Context enrichment**; a fresh overnight queue is informational. Scheduler and Sources retain expandable diagnostics, but recommendation stderr is withheld in favor of its sanitized structured receipt. Health refresh is status-only; the separate `Check sources` action runs a bounded live ingest for the selected source or all hourly sources, then revalidates the local Library view.
...
- `CommentPopover` is compact capture by default. Enter posts a queued message and keeps the popover open. `Chat now` grows the house `ObjectPopover` to conversation size and hosts the same `ThreadDrawer` used by Chats, so context never disappears behind a forced navigation.
- `ThreadView` remains the dense, under-object record. Its legacy inline process affordance can hand off to Chats, while the universal popover owns the preferred in-place start.
- `ThreadDrawer` is the live surface in both pane and popover hosts: `ThreadConversationTimeline` adapts file-native thread messages/outcomes and attached chat evidence into the same `ConversationTurn` grammar used by free chats. Human bubbles align right, assistant prose aligns left, source metadata links back to navigable Hilt artifacts, and each run's `ChatTracePanel` sits directly under the assistant reply it explains. Its shared `ChatComposer` starts a turn immediately and keeps Send available beside Stop so a message posted during an active turn becomes the next volley. Closing the host does not explicitly abort the draining request; Stop does.
...
- `ChatsView` derives attention from queued messages, dev-item state, running work, and unread replies. Thread rows reserve the left column for title + latest-message preview and put compact age/state/count in a right-aligned two-line rail. A merely open but handled conversation stays in All without polluting Needs you; Done means explicitly closed or archived.
