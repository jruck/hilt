...
- **Performance subtab simplified to a single-axis closet-climate chart** — The Mercury Performance view dropped everything that wasn't a temperature: power-draw wash, GPU/Mem utilization, Load, Humidity, the Detail/Util progressive-disclosure layers, and the dual hot/cool axes. Rationale: utilization/power are upstream proxies for the heat that *Compute* already expresses, so with them gone all three remaining series (Closet, Outdoor, Compute) are °F and belong on **one shared axis** — which makes the vertical relationship read true (compute rides above closet rides above outdoor on the same scale, instead of being an artifact of separate axes). **Closet is the hero**: a thick line **color-coded along its length by zone** via a vertical SVG gradient with hard stops — green below 85°F, yellow 85–90°F, red at/above the 90°F ceiling — so the line itself changes color as it crosses the thresholds rather than being one flat color. Faint zone **bands** back the 85/90 thresholds (the red band is capped a few degrees above 90 so it marks the danger zone without washing the upper chart where compute lives) plus a labeled "90° ceiling" reference line. **Outdoor** is now a pastel-blue filled backdrop, bridged across gaps so its sparser sampling reads as a continuous wash instead of a broken line. **Compute** stays a thin violet line. The stat strip collapses to the three series (closet's swatch tracks its live zone color); muting Compute re-fits the axis to just the ambient band, making closet maximally dramatic. Thresholds live in `PerfChart.tsx` (`CLOSET_WARN`/`CLOSET_CEILING`/`ZONE`/`closetZoneColor`). Data plumbing (Mercury proxy routes + SWR) is unchanged — this is purely the visualization layer.
