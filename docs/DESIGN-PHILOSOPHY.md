...
- Camera-wall tiles can use a larger `rounded-2xl` radius and pronounced shadow, with matching rounded overlay chips/buttons, so preview cards read as lifted monitor tiles without becoming decorative.
