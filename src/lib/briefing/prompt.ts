...
- GRANULARITY: cite at exactly the granularity the data gives. A meeting filename's start timestamp doesn't tell you its length ("90-minute session" was graded fabrication); a commit subject doesn't tell you which files it touched. With start times only, say "back-to-back" or "may collide" — never assert "double-booked"/"overlaps" as fact without end times.
