...
- NEXT-STEPS MECHANICS (non-negotiable, daily mode): in ⏭ Next steps, each meeting entry is ONE bullet (bold substance lead), its meeting citation as the first sub-bullet, then that meeting's pending proposal task ids as sub-bullets — exactly one \`t-YYYYMMDD-NNN\` per line, in backticks, no other prose on an id line (the reader UI replaces the line with the live task card). Never restate a proposal's title/quote/details in prose — the card renders them. Only ids present in the gathered data may appear; never invent or derive one.
- PILL CITATIONS: cite system objects as inline links with the \`hilt:\` scheme — \`[name](<hilt:kind/id>) — ALWAYS wrap the destination in angle brackets: vault paths contain spaces/parens which break plain links\` (kinds: \`meeting\` id = vault-relative note path, \`task\` id = \`t-YYYYMMDD-NNN\`, \`person\` id = slug, \`project\` id = vault-relative dir, \`library\` id = artifact id). The reader renders these as object pills (hover-preview + click-through). A bullet whose SINGLE citation is the object it is about carries the pill inline in the bullet's own line — no separate citation sub-line. Detail sub-bullets must be EVIDENCE (a fact, a quote, a delta, a date) — never a bare citation line that only repeats the source; the pill already carries the source. Existing join keys are unchanged: the ⏭ meeting-citation first sub-bullet, \`*loop:<id>, <date>*\`, and backticked task-id lines stay exactly as specified. Only ids present in the gathered data may appear in a hilt: link — never invent one.
