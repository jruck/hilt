...
- `npm run library:series:synthesize -- --series-id <id> --write` creates or refreshes a parent series note from child Library items. Use `--source <source-id>` for source-driven series, omit `--write` for a dry run, and add `--analyze` after child reweaves complete to run the vault-aware whole-series synthesis pass.
