...
### 2026-07-18: Expensive Experiments Leave a Time Capsule, Not Product Cruft

**Principle**: When an aspirational system is not earning its operating cost, preserve enough evidence to revisit the idea and then remove it completely from the live product. Do not leave dead tabs, stale links, hidden runners, compatibility terminology, or duplicate explanations behind merely because the experiment may be interesting again later.

For Library scoring, the replacement should be deterministic and legible at the user's altitude. Keep Worth compact; call its contextual component Current fit; explain matched current work, important terms, explicit Connections, and attention adjustment in ordinary language; keep versions and score snapshots one disclosure deeper. Historical records stay historically true, while current screens use the current scoring method. Preservation happens in an isolated source branch plus a private data capsule—not through a permanent shadow evaluator or an ongoing user training obligation.
...
### 2026-05-30: Knowledge Graph — Historical Design, Retired 2026-07-18

This section records the original design reasoning for the archived experiment; it no longer describes live Hilt. The backend semantic overlay, visual graph, and cross-app graph links were retired after a historical bake-off showed that deterministic Current-fit scoring could preserve the valuable Library behavior without ongoing Gemini cost. See the [semantic graph v1 tombstone](retired/semantic-graph-v1.md).
...
- **"Show in graph" was a peer affordance, not a separate destination.** Docs, Library, and People deep-linked to a focused graph node. Those actions were removed with the feature so no dead or misleading affordances remain.
- **Retirement is complete, not a hidden flag.** Current Hilt has no knowledge-graph renderer, routes, runner, databases, or graph links. The source and private data were preserved first so future research starts from evidence instead of abandoned production cruft.
...
- Library eval metrics use progressive disclosure. Normal Recent/New reading surfaces show only the compact worth score (`Zap` icon) as the priority signal in the card action row. It should look like an inline count/action, not a blue pill; clicking it opens the reference and expands the metadata panel. Store and compute eval metrics as 0–1 decimals, but render them in the UI as integer 0–100 scores (`0.87` → `87`) so cards and metadata stay legible. Admin/eval review contexts expand the same action row into component metrics: worth (`Zap`), Current fit (`Target`), substance (`Layers`), and freshness (`Clock`), plus an `Archive?` lifecycle flag only when the eval suggests `to_archive`. The expanded explanation should name matched current work, important terms, explicit active connections, and attention adjustment; raw versions stay collapsed under Technical details. Avoid artificial bucket labels such as `Must Read`, `Recommended`, or `Interesting`. Generic eval `why` stays in detail; a deliberately selected For You episode instead leads with its editor-written `Why now`, kept separate from the source summary.
...
### 2026-07-10: For You Is One Durable Personal Attention Feed

**Change**: Replaced the temporary eight-item score ranking with an ongoing recommendation stream shared by Library and the daily briefing. Editorial runs write recommendation episodes; the feed shows one card per artifact using its latest episode, so a genuinely relevant older item can return to the top with a new reason without creating a duplicate.

**Pattern established**:
- For You is a single chronological attention feed. Do not break it into `Now`, `Earlier`, or batch sections; today adds to yesterday, and resurfacing moves the same stable card.
- Recommendation order changes only when the editor writes a new episode. Reading, ordinary score drift, and metadata refreshes must not move cards.
- Treat source description and recommendation pitch as two stable editorial roles, then show only the role appropriate to the surface. The source description is evergreen and explains the item; the pitch is contextual and explains the current consequence. For You and Briefing use the pitch as the sole unlabeled description. Recent/New/Search use only the source description plus a quiet `In For You` signal. Detail is the one reading surface that deliberately separates both.
- Lead recommendations with a specific, non-salesy pitch grounded in a changed save, meeting, task, project, or briefing signal. Reject repeated evidence, materially unchanged pitches, and pitches that merely paraphrase the source description.
- Dismissal applies to recommendation state, not saved/candidate lifecycle. Keep comment feedback in the same Library thread and make Undo immediate.
- The daily briefing is the short front door to the same feed: at most three new unread episodes, rendered as compact inset cards inside the Library section, then a native `View all` into For You. Never pad with stale recommendations.
- Preserve attention and place during live updates. Near the top, insert immediately; deeper down, hold the visible-card anchor and offer a quiet new-items control.
- Numeric eval scores are diagnostic evidence, not the recommendation's headline. Normal recommendation cards should explain timing and usefulness in words; detailed/admin surfaces retain the numbers.
- In `Library & knowledge`, information architecture should express editorial hierarchy rather than flattening everything into prose: recommendations form one card cluster with their set lead and `View all`; the weekly memo is its own prominent band; daily health is a quiet footer. Never nest full FeedCards inside another elevated card, and never let operational health lead the section when there is reading to recommend.
